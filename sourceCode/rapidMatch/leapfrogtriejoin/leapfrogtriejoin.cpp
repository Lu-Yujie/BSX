...
bool leapfrogtriejoin::execute(size_t& call_cnt, size_t& embedding_cnt, int64_t& time_limit) {
    ...
    for (uint64_t i = 0; i < input_->get_size(); ++i) {
        ...
        if (start_depth >= num_core_vertex_) {
            ...
            if (enumerate_non_core_results(start_depth, max_depth, call_cnt, embedding_cnt, time_limit))
                return true;
            ...
            while (true) {
                while (idx_[cur_depth] < num_local_candidates_[cur_depth]) {
                    ...
                    if (cur_depth == max_depth - 2) {
                        ...
                        if (embedding_cnt >= output_count_limit_) {
                            return false;
                        }
                    }
                        ...
                        if (enumerate_non_core_results(next_depth, max_depth, call_cnt, embedding_cnt, time_limit))
                            return true;
#ifdef FAILING_SET_PRUNING
                        if (!vec_failing_set_[cur_depth].test(vertex_ordering_[cur_depth])) {
                            vec_failing_set_[cur_depth - 1] = vec_failing_set_[cur_depth];
...
bool leapfrogtriejoin::enumerate_non_core_results(uint32_t start_depth, uint32_t max_depth, size_t& call_cnt,
                                                  size_t& embedding_cnt, int64_t& time_limit) {
