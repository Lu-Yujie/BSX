...
void GenerateQueryPlan::generateQSIQueryPlan(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix,
      ...
      * 每一条边的weight用w(e (u,u′)) = |{e (v,v′) ∈ E(G)|L(v) = L(u) ∧ L(v′) = L(u′)}|
      */
     std::vector<bool> visited_vertices(query_graph->getVerticesCount(), false);
     std::vector<bool> adjacent_vertices(query_graph->getVerticesCount(), false);
     order = new ui[query_graph->getVerticesCount()];
...
// 读一读论文再看吧，TurboISO的具体算法综述里是一点没提
void GenerateQueryPlan::generateTSOQueryPlan(const Graph *query_graph, Edges ***edge_matrix, ui *&order, ui *&pivot,
    ...
    for (std::vector<ui>& path : paths) {
        ...
        std::vector<uint64_t> estimated_embeddings_num;
...
// 这个破算法，真的折磨人，好复杂好复杂
// 简单来说，就是根据root-to-leaf path来排序，思想还是优先剪枝
void GenerateQueryPlan::generateCFLQueryPlan(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix,
    ...
    if (!core_paths.empty()) {
        std::vector<std::vector<uint64_t>> paths_embededdings_num;
        ...
        for (auto& path : core_paths) {
            ...
            std::vector<uint64_t> path_embeddings_num;
...
// 讲真，这个代码我没看明白，这个感觉和综述里讲的生成顺序的算法不太一样
    ...
    for (int i = q_num - 1; i >= 0; --i) {
        ...
        for (ui j = 0; j < node.fn_count_; ++j) {
            ...
            if (child_node.bn_count_ == 1) { // the tree-like contraint
                ...
                // 对于两组candidate，计算vertex每个candidate连接另一组candidate的weight的和
...
// 排序顺序：
//   1. 和已经在order中的点相连最多的
//   2. 和已经在order中的点共享邻居最多的（就是再往外看一跳）
//   3. 既不在order中，也不和order中点相邻的邻居最多的
//      （就是看下一个点的邻居与order中已有点的紧密度，越疏远越优先）
void GenerateQueryPlan::generateRIQueryPlan(const Graph *data_graph, const Graph *query_graph, ui *&order, ui *&pivot) {
    ui q_num = query_graph->getVerticesCount();
    order = new ui[q_num];
    pivot = new ui[q_num];

    std::vector<bool> visited(q_num, false);
...
// 起始点: label最稀有，degree最大
// 其他点: 
//      1.生成一个bfs树，按照层级添加点
//      2.对于每一层的节点，优先选择和order中已有点连接对多的
//      3.选择degree最大的
//      4.选择数据图上对应label最少的
void
GenerateQueryPlan::generateVF2PPQueryPlan(const Graph *data_graph, const Graph *query_graph, ui *&order, ui *&pivot) {
    ...
    ui property_count = 0;
    std::vector<std::vector<ui>> properties(q_num);
    std::vector<bool> order_type(q_num, true);     // True: Ascending, False: Descending.
    std::vector<ui> vertices;
    std::vector<bool> in_matching_order(q_num, false);
    ...
    while (num_vertices_in_matching_order < q_num) {
...
GenerateQueryPlan::generateVF3QueryPlan(const Graph *data_graph, const Graph *query_graph, ui *&order, ui *&pivot) {
    ...
    memset(pivot, -1, q_num*sizeof(ui));

    // 计算节点匹配概率
    double* prob = new double[q_num];
    computeProbability(data_graph, query_graph, prob);

    // 排序指标
    std::vector<std::vector<double>> properties(q_num);
    std::vector<bool> order_type(q_num, true);     // True: Ascending, False: Descending.
    std::vector<ui> vertices(q_num);

    for (ui i = 0; i < q_num; ++i) {
        vertices[i] = i;              // 初始化匹配顺序
        properties[i].resize(3);      // 一共有三个排序指标
        properties[i][0] = 0;         // 初始时，已匹配点集为空（会随着匹配进行而变化）
        properties[i][1] = prob[i];   // 匹配成功概率（不会变化）
        properties[i][2] = data_graph->getVertexDegree(query_graph->getVertexLabel(i)); // 节点的度信息（不会变化）
    }
    ...
    for (ui i = 1; i < q_num; i++) {
        ...
        for (ui j = 0; j < un_count; j++) {
            properties[un[j]][0] += 1; // 将上一个匹配的点的邻居信息更新到properties中
        }
    ...
    for (ui i=0;i<q_num;i++) {
        ...
        ui degree_num = degree > max_degree ? 
                        ...
                        : d_num - (degree != 0 ?
                                               degrees[degree - 1] 
                                               : 0);
        prob[i] = ((double)label_num / (double)d_num)
                    * ((double)degree_num / (double)d_num);
    }
