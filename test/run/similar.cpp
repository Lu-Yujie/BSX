...
void constructCommands() {
    std::string exefile = "/home/yujielu/subgraph/bsx/code/reviews/similar/build/bin/BS";
    std::string con_path = "/home/yujielu/subgraph/bsx/code/reviews/similar/test/conf";
    std::string file_suffix = ".txt";
    int time_limit = 1000;
    std::string max_num = "MAX";
    int num = 1000;
    int parallel_num = MAX_CONCURRENT_COMMANDS;
    std::string similar_ratio = "25";  // 25, 50, 75, 100
    ...
    for (const auto &graph : graphs) {
        ...
        std::string outputspace = "/var/lib/docker/subgraph/output/bsx/empty/similar/s" + similar_ratio + "/" + graph;
...
// how to stop
// ps -ef | grep -E "BS|similar" | grep -v grep | cut -c 9-16 | xargs kill -9
