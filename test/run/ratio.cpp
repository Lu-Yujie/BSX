...
void constructCommands() {
    ...
    std::vector<int> lsizes = {10, 50, 100, 200, 1000, 10000, 100000};
    std::vector<int> sizes = {10, 20, 30, 40, 50};

    for (const auto &graph : graphs) {
        std::string dataspace = "/var/lib/docker/subgraph/dataset/slabel/nlabel/" + graph;
        std::string outputspace = "/var/lib/docker/subgraph/output/bsx/empty/" + graph;

        for (const auto &lsize : lsizes) {
            std::string data_graph = "/B" + std::to_string(lsize) + "/" + graph + "-" + std::to_string(lsize) + file_suffix;
            for (const auto &size : sizes) {
                for (int j1 = 0; j1 <= num / parallel_num; ++j1) {
                    for (int j2 = 1; j2 <= parallel_num && j1 * parallel_num + j2 <= num; ++j2) {
                        int i = j1 * parallel_num + j2;
                        std::string query_graph = "/B" + std::to_string(lsize) + "/Q" + std::to_string(size) + "/Q" + std::to_string(size) + "-" + std::to_string(i) + file_suffix;
                        std::string output_file = "/B" + std::to_string(lsize) + "/Q" + std::to_string(size) + "/" + std::to_string(j2) + ".csv";
...
// how to stop
// ps -ef | grep -E "BS|ratio_label" | grep -v grep | cut -c 9-16 | xargs kill -9
